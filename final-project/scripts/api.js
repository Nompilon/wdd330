document.addEventListener("DOMContentLoaded", () => {
    ...
    async function getFoodData(foodName) {
        try {
            const res = await fetch(`https://api.nal.usda.gov/fdc/v1/foods/search?query=${encodeURIComponent(foodName)}&pageSize=1&api_key=${API_KEY}`);
    ...
    function getNutrientGoals() {
        return JSON.parse(localStorage.getItem("nutrientGoals")) || { calories: 2000, protein: 150, carbs: 250, fat: 70 };
    }
    ...
    foodInput.addEventListener("input", () => {
        ...
        suggestionTimeout = setTimeout(async () => {
            try {
                const res = await fetch(`https://api.nal.usda.gov/fdc/v1/foods/search?query=${encodeURIComponent(query)}&pageSize=10&api_key=${API_KEY}`);
                const data = await res.json();
    ...
    mealForm.addEventListener('submit', async (e) => {
        ...
        if (!foodName || isNaN(portion) || portion <= 0) return alert("Enter valid food name and portion.");
